...
void LaserCB(const sensor_msgs::LaserScan::ConstPtr& scan){
    ...
    // POINTS markers use x and y scale for width/height respectively
        ...
        //skozi vrsto
           ...
           //alfa = atan(xOdmik/yOdmik);


           cout << xOdmik << " " << yOdmik << " " << mer_L.size() << " " << mer_D.size()<< endl;
    /*
           if(yOdmik >= ySredina - 2.0 && yOdmik <= ySredina + 2.0){

               if(xOdmik >= xSredina - 2.0 && xOdmik <= xSredina + 2.0){
                   zSmer = 0;
               }else{

                   if(xOdmik > xSredina){

                   }else if(xOdmik < xSredina){

                   }

               }

           }else if(yOdmik > ySredina){
...
int main(int argc, char **argv){
    ...
    if(nh.hasParam("/navigation_task1/obrni") && nh.hasParam("/navigation_task1/min_val") && nh.hasParam("/navigation_task1/max_val")){
        ...
        nh.getParam("/nagivation_task1/xSredina", xSredina);
        nh.getParam("/navigation_task1/ySredina", ySredina);
    }else{
