...
void LaserCB(const sensor_msgs::LaserScan::ConstPtr& scan){
    ...
    if(izvajanje == 0)
    {
        float desna = 0, leva = 0, smer = 0, hitrost = 0, sredina = 0.12;
        int delitelj = 0;
        ...
        if (delitelj <2) desna = 0;
        //else if (delitelj <5) desna = 0.63;
        else desna = desna / delitelj;
    
        cout << "Desna: " << desna << "size: "<< scan->ranges.size() <<endl;
        delitelj = 0;
        ...
        for(int l = (scan->ranges.size()/2+scan->ranges.size()*sredina/2); l <= scan->ranges.size(); l++){
            ...
            if(scan->ranges[l] < 2 && scan->ranges[l] > -2){
                //leva +=  cos((21+(-1*(l-scan->ranges.size())))) * scan->ranges[l];
                leva += scan->ranges[l];
                delitelj++;
            }
        }
        if (delitelj <2) leva = 0;
    ...
    float x = (desna - leva)*1.3;
        ...
        if(leva > 0.001 && desna > 0.001){
            ...
            if(x >= 0)//x je pozitiven , desna je vecja razdalja pojdi desno
            {
                ...
                /*hitrost = 0.8687*x*x*x+1.8248*x*x+1.3247*x+0.5183;
                smer  = 0.5297*x;*/
                hitrost = 0.5;
                smer = -0.3*x/0.2; // leva + |||||   denso -
            }
    ...
    /*  if (smer < 0.1 && smer > -0.1){
            hitrost = 0.5;
        }
        else{
            hitrost = 0.4 - (abs(smer)*0.333);
        }*/
        //smer = smer * -1;
