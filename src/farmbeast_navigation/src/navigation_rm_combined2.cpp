        ...
        //skozi vrsto
            ...
            if(mer_L.size() <2){
                for(int l = 75; l <= 75 + st_mer && l <= 134; l++){
                    if((scan->ranges[l] > min_val)&&(scan->ranges[l] < max_val)){
                        mer_L.push_back((scan->ranges[l])*(-1)); //vrednosti morajo biti negativne, saj bi bile tudi na koordinatnem sistemu
                        kot_L.push_back(l - 75); //pretvorba kota za levo stran
                    }
                }
...
/*
           ...
           /* zSmer = zSmer*max_obr;
            ...
            if(mer_D.size() == 0 && mer_L.size() == 0){
                            ...
                            if(povprecje_tren - povprecje_pos >= min_dolzina){
                                ustavi = true;
            ...
            if(!leva_niT && !desna_niT){ //imamo obe strani
                if(absPovpL >= (absPovpD - napaka) && absPovpL <= (absPovpD + napaka)){ //če smo na sredini

                    //LEVA
                    for(int i = 0; i < mer_L.size(); i++){
                        xsumL = xsumL + mer_L[i];
                        ysumL = ysumL + kot_L[i];
                        x2sumL = x2sumL + mer_L[i]*mer_L[i];
                        xysumL = xysumL + mer_L[i]*kot_L[i];
                    }
...
int main(int argc, char **argv){
    ...
    nh.setParam("/navigation_task2/kot_cal", 1);

    if(nh.hasParam("/navigation_task2/obrni") && nh.hasParam("/navigation_task2/min_val") && nh.hasParam("/navigation_task2/max_val")){

        nh.getParam("/navigation_task2/min_val", min_val);
        nh.getParam("/navigation_task2/max_val", max_val);
        nh.getParam("/navigation_task2/obrni", obrni);
        nh.getParam("/farmbeast_auto", robot_run);
        nh.getParam("/farmbeast_task_select", farmbeast_task_select);
        nh.getParam("/navigation_task2/max_obr", max_obr);
        nh.getParam("/navigation_task2/xSmer", xSmer);
        nh.getParam("/navigation_task2/st_mer", st_mer);
        nh.getParam("/navigation_task2/hitrost", hitrost);
        nh.getParam("/navigation_task2/koef", koeficient);
        nh.getParam("/navigation_task2/ack_nac", ack_nac);
        nh.getParam("/PhidgetsImuNodelet/nac_voznje", voznja_nacin);
        nh.getParam("/navigation_task2/min_dolzina", min_dolzina);
    }else{
